...
import { MoviePosterCard, MovieThumbCarousel, Pagination } from "@/components/ui";
...
function PhimChieuRapContent() {
  ...
  const { data: latestData } = useMovieList({
  ...
  const { data, isError, isFetching } = useMovieList(
    { typeList: "phim-chieu-rap", page, limit: 24 },
    { placeholderData: keepPreviousData }
  );

  const latestItems = latestData?.data?.items ?? [];
  const titlePage = data?.data?.titlePage ?? "Phim chiếu rạp";
  ...
  return (
    <PageLayout className="pb-24">
      ...
      <MovieThumbCarousel
        title="Phim chiếu mới nhất"
        titleId="latest-cinema-heading"
        items={latestItems}
        navId="latest-cinema"
        className="mb-12"
      />

      {/* Phim chiếu rạp đề xuất — Grid */}
      <section aria-labelledby="recommended-cinema-heading">
        <h2
          id="recommended-cinema-heading"
          className="mb-4 text-lg font-semibold text-[var(--foreground)]"
        >
          Phim chiếu rạp đề xuất
        </h2>
        ...
        {items.length > 0 && (
          <div ref={listRef} className="relative">
            ...
            <Pagination
              ...
              getPageHref={(p) => `/phim-chieu-rap?page=${p}`}
              className="mt-8"
            />
          </div>
        )}
  ...
  <PageLayout className="pb-24">
    ...
    <div className="grid grid-cols-3 gap-4 lg:grid-cols-8">
      {Array.from({ length: 12 }).map((_, i) => (
        <div key={i} className="aspect-[2/3] animate-pulse rounded-[var(--radius-panel)] bg-[var(--secondary-bg-solid)]" />
      ))}
    </div>
  </PageLayout>
);
